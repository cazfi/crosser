...
if is_minimum_version "${VERSION_SQLITE}" 3.51.2
then
  SQL_SUBDIR="2026/"
elif is_minimum_version "${VERSION_SQLITE}" 3.48.0
then
  SQL_SUBDIR="2025/"
elif is_minimum_version "${VERSION_SQLITE}" 3.45.0
then
  SQL_SUBDIR="2024/"
...
RET="${RET} $?"
download_needed "https://pkgconfig.freedesktop.org/releases/" "pkg-config" "${VERSION_PKG_CONFIG}" "tar.gz"
RET="${RET} $?"
...
RET="${RET} $?"
download_needed "https://tango.freedesktop.org/releases/" "icon-naming-utils" "${VERSION_ICON_NUTILS}" "tar.bz2"
RET="${RET} $?"
download_needed "https://tango.freedesktop.org/releases/" "tango-icon-theme" "${VERSION_TANGO_ICONS}" "tar.bz2"
RET="${RET} $?"
...
RET="${RET} $?"
download_needed "${MIRROR_GITHUB}/libsdl-org/SDL_mixer/releases/download/release-${VERSION_SDL3_MIXER}/" "SDL3_mixer" "${VERSION_SDL3_MIXER}" "tar.gz"
RET="${RET} $?"
